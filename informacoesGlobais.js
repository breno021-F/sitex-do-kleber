...
async function visualizarInformacoesGlobais() {
    ...
    paragrafo.innerHTML = `O mundo tem <span>${pessoasMundo}</span> bilhões de pessoas. Dessas, aproximadamente <span>${trabalhadoresMundo}</span> bilhões estão empregadas e passam em média <span>${tempoDesTrabalho} horas</span> e <span>${minutos} minutos</span> por dia no caminho para o trabalho. Temos, portanto, mais da metade da população mundial que não exerce trabalhos com vínculos empregatícios legais. Isso pode significar que há longevidade e, desse modo, muitas pessoas aposentadas. Porém, também pode significar muitos postos de trabalho chamados informais.`;
